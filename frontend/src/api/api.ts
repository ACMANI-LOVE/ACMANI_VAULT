...
export const putConfigsAPI    = ({request}:APIRequest) => { fetcher("PUT" , `http://localhost:8000/configs/`, request)}
...
export const deleteGroupsAPI = ({prmText,request}:APIRequest) => { fetcher("DELETE", `http://localhost:8000/groups/${prmText}`, request)}
